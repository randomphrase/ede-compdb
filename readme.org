...
In this (admittedly complex) example, we have a possible four different types of build for each project. Each build type is assigned a separate directory, relative to the project root. 
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-load-cmake-project (dir)
      ...
      (let ((config-and-dir (car (cl-member-if 
                                  (lambda (c)
                                    (file-readable-p (expand-file-name "compile_commands.json"
                                                                       (file-name-as-directory (cdr c)))))
...
The ~my-cmake-build-directories~ variable describes the possible build types and their corresponding directories. Before loading the EDE-compdb project, we determine which build directory is present. From this we infer the build type, which corresponds to the ~:configuration-default~ slot.
